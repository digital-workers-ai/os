...
`unit` runs inside the backend container and starts the stack when it is down. `snap` builds a fresh stack under its own compose project and takes about nine minutes: run it alone, never overlapping the unit suite, whenever anything under `app/frontend/` changes, and ship the updated baselines in the same PR. Nothing is committed on red.
