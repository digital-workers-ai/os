...
- `.claude/skills/` — `_implement`, `_ship`, `_add-tests`, the conventions for anyone working in the repo; `_operator` and `_operator-build`, the two agents' rules
- `definitions/` — the definition files (`ontology.yaml` through `derived.yaml`), `briefs/`, the role prompt files, and `operator.md`, the operator's brief, beside `briefs/` so it is not a role
