...
Every tracker is an ordinary connector. The vendor's answer lands verbatim in the raw store, and a rebuild projects it into four entities, `visibility_check`, `visibility_mention`, `ad` and `competitor_post`, through the same mappings and transforms as everything else, so every count on the page traces to a source record. There is one check per engine per query per day. Mention detection is deterministic: whole-word matches on each company's name and aliases, plus the domains the answer cites. It re-runs on every rebuild, so editing `spy.yaml` and rebuilding re-scores the history. On Google the rank is the organic position; everywhere else it is the order the company is mentioned in. AI Overviews come from the same SerpApi search, fetched with a follow-up call when Google defers them. Ad text is read by GPT from the creative's preview image; video creatives stay nameless. LinkedIn posts arrive through Bright Data's asynchronous dataset, trigger, poll, download, and are public data only. The metrics live in `metrics.yaml` like any other, and a rule flags every check of the last seven days where the brand is absent.
