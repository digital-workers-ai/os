...
To verify a source against its real API, set its variables in `app/.env` — HubSpot reads `HUBSPOT_ACCESS_TOKEN`, with `HUBSPOT_BASE_URL` as an optional override — and run `docker compose -f app/docker-compose.yml run --rm backend python -m tools.pull_source hubspot --compare --capture`. It prints, per pull, the requests made, the records returned and how many the mappings accepted, the mapped paths no record carried, and the skips by reason; `--compare` lists the keys and JSON types that differ from the stand-in fixture, and `--capture` lands the raw records under `app/backend/captures/`, which is gitignored because it holds your data. Once the compare is clean and personal data is scrubbed, a person moves the capture to `app/backend/fixtures/real/<source>/` and records `expected.json` there, which is what makes the source provider-validated. With no variable set the same command exercises the stand-in, and once all of a source's variables are set every sync reads the real API too.
