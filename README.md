...
Every metric and entity may also carry a `description` and `synonyms`, and `ontology.yaml` carries a top-level `attributes:` map glossing the labels themselves — the one place a name is given a meaning, read by the console, the ask agent, MCP and search, so `revenue` finds `mrr` and nobody has to guess what `external_ref` holds. A synonym that collides with a name or another synonym fails the build: an alias that points at two things is worse than no alias.
