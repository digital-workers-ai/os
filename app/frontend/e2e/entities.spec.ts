...
test('raw dropdowns open', async ({ page }) => {
  ...
  const { entities } = await (await page.request.get('/api/records?limit=500')).json()
  const sources = [...new Set<string>(entities.map((r: { source: string }) => r.source))].sort()
  await openFilter(page, 'records-source-filter')
  await expect(page.getByTestId('records-source-filter-option')).toHaveText(['all sources', ...sources])
