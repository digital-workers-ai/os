...
import { isRanged } from '@/lib/pages'
...
export function Layout({ today, range, onRange }: { today: string | null; range: Range; onRange: (range: Range) => void }) {
  const page = useMatch('/:page/*')?.params.page
  const ranged = isRanged(page) && !!today
