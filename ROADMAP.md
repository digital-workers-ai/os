...
- [x] 9. feat: metrics remember — `POST /api/metrics/snapshots` is the one history writer (deliberately not the rebuild: a series must count scheduled runs, not pipeline runs), `GET /api/metrics/history`. Brings: `metric_snapshot` table, snapshot retention. Fixes: E1, E6 (E5 is inferred-only code — moved to 21).
