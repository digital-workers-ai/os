...
- [x] 21. feat: inferred metrics — metrics over `enriched_fact`, digest-stamped snapshots. Fixes: E4 (guard written fresh — v12 never had window grammar, and unknown keys were silently ignored, E4's shape reborn), E5 (produced_by scoped to counted rows). Brings: terms/op ratio grammar with both shapes parsing, COUNT_DISTINCT on enriched terms only, runs/comparable/breaks history + GET /api/metrics/history/{metric}, snapshot lineage columns, goals refusing incomparable series, checks exemptions, entity-must-match-reading guard (v11 gap). Deferred with their grammar: group_by (own-attr + edge-hop) and window_days.
- [x] 22. feat: a role gets a briefing — coaching briefer + prompts + API, fenced estate data. Fixes: H1 in v12 form (at_most says over-the-ceiling, band reports its band; streak/dropped_out_of_bounds branches have no v12 producer). Prompts tenant-editable at repo-root prompts/; COACHING_MODEL=claude-sonnet-5 (cost symmetry — Opus one env var away, no prose scoreboard exists); briefings stored with lineage, failures leave rows; role GET+POST share one route so the 405 Allow header is truthful.
- [x] 23. feat: ask the estate a question — conversation agent + tools + threads + API. Fixes: H2 (narrowable metrics tool — pinned honestly: 14 metrics fit the cap, so truncation is proven synthetically), J11 (both halves: readable bad-arg errors; history server-side so a forged history key is ignored). Also fixed unfiled: a max_tokens-truncated final answer says truncated. Cut per land-with-first-consumer: thread list + DELETE routes. System prompt stays in code (engine invariants, not tenant voice); CONVERSATION_MODEL=claude-sonnet-5 — the loop's growing context makes this the layer where model cost bites hardest.
