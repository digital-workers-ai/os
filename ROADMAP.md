...
- [x] 5. feat: sync HubSpot — `POST /api/sync` pulls companies/contacts/deals into `raw_event`; `GET /api/raw` and `/api/raw/manifests` show rows and manifests. Brings: client, creds, registry, hubspot connector, sync.py, `sync_run` + `pull_manifest` tables, raw/sources APIs. Fixes: defect 1 (lookback poisons tombstones), A4, A5, F2 (handler half), J3, J4/F3.
